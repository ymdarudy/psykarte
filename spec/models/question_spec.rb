...
RSpec.describe Question, type: :model do
  describe "validation" do
    example "titleは必須" do
      ...
      question = build(:question, title: "設問1")
      expect(question).to be_valid
    end

    example "choice_min_pointは必須、かつ整数のみ" do
      question = build(:question, choice_min_point: "")
      expect(question).to be_invalid

      question = build(:question, choice_min_point: 1.5)
      expect(question).to be_invalid

      question = build(:question, choice_min_point: 2)
      expect(question).to be_valid
    end

    example "choice_max_pointは必須、かつ整数のみ" do
      question = build(:question, choice_max_point: "")
      expect(question).to be_invalid

      question = build(:question, choice_max_point: 3.5)
      expect(question).to be_invalid

      question = build(:question, choice_max_point: 4)
      expect(question).to be_valid
    end

    example "choice_min_wordは必須、かつ30文字以内" do
      question = build(:question, choice_min_word: "")
      expect(question).to be_invalid

      question = build(:question, choice_min_word: "a" * 31)
      expect(question).to be_invalid

      question = build(:question, choice_min_word: "下限文言")
      expect(question).to be_valid
    end

    example "choice_max_wordは必須、かつ30文字以内" do
      question = build(:question, choice_max_word: "")
      expect(question).to be_invalid

      question = build(:question, choice_max_word: "a" * 31)
      expect(question).to be_invalid

      question = build(:question, choice_max_word: "上限文言")
      expect(question).to be_valid
    end

    example "point_reversalはtrue,またはfalse" do
      question = build(:question, point_reversal: "")
      expect(question).to be_invalid

      question = build(:question, point_reversal: nil)
      expect(question).to be_invalid

      question = build(:question, point_reversal: true)
      expect(question).to be_valid

      question = build(:question, point_reversal: false)
      expect(question).to be_valid
    end
  end
